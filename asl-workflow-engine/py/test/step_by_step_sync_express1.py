...
#
# Run with:
# PYTHONPATH=.. python3 step_by_step_sync_express1.py
#
"""
...
This example is more or less identical to step_by_step_sync_express.py but in
this example the caller state machine is invoked using the StartSyncExecution
API call so the client invocation will block until the state machine completes.
https://docs.aws.amazon.com/step-functions/latest/apireference/API_StartSyncExecution.html

***** IMPORTANT NOTE *****
Using boto3 (and probably other AWS SDKs) to call the StartSyncExecution API
has a subtle quirk whereby it does "host prefix injection" for some operations
so given an endpoint_url="http://localhost:4584" boto3 will attempt to call
http://sync-localhost:4584/ and will fail with Could not connect to the
endpoint URL: "http://sync-localhost:4584/"
So it is necessary to disable host prefix injection by setting
config=botocore.config.Config(inject_host_prefix=False) when creating the client
in other words:
    sfn = boto3.client("stepfunctions", endpoint_url="http://localhost:4584",
                       config=botocore.config.Config(inject_host_prefix=False))

see:
https://github.com/localstack/localstack/issues/5258#issuecomment-1110175700
https://boto3.amazonaws.com/v1/documentation/api/latest/reference/services/stepfunctions/client/start_sync_execution.html
https://github.com/aws/aws-cli/issues/6898
...
if __name__ == '__main__':
    # Initialise logger
    logger = init_logging(log_name='step_by_step_sync_express1')
    ...
    create_tracer("step_by_step_sync_express1", {"implementation": "Jaeger"})

    # Initialise the boto3 client setting the endpoint_url to our local
    # ASL Workflow Engine
    # https://boto3.amazonaws.com/v1/documentation/api/latest/reference/core/session.html#boto3.session.Session.client
    # Note settig inject_host_prefix=False is required for start_sync_execution
    # or boto3 will attempt to use the endpoint http://sync-localhost:4584
    sfn = boto3.client("stepfunctions", endpoint_url="http://localhost:4584",
                       config=botocore.config.Config(inject_host_prefix=False))

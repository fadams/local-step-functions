...
#
# Run with:
# PYTHONPATH=.. python3 step_by_step_sync1.py
#
"""
...
"fire and forget" manner so the parent execution of the sync1_caller_state_machine
...
Note that using the resource startExecution.sync causes the Output of the
nested state machine to be returned to the calling Task as a String.
In this example we convert the Output to JSON using a ResultSelector with a
States.StringToJson Intrinsic. If JSON Output is required it is likely better
to use startExecution.sync:2 rather than converting as done here.

Note too that the output of the nested state machine is stored in the "Output"
...
if __name__ == '__main__':
    # Initialise logger
    logger = init_logging(log_name='step_by_step_sync1')
    ...
    create_tracer("step_by_step_sync1", {"implementation": "Jaeger"})
    ...
    sync1_caller_state_machine_arn = "arn:aws:states:local:0123456789:stateMachine:sync1_caller_state_machine"
    ...
    def create_state_machines():
        ...
        try:
            ...
            response = sfn.create_state_machine(
                name="sync1_caller_state_machine", definition=caller_ASL,
            ...
            response = sfn.update_state_machine(
                stateMachineArn=sync1_caller_state_machine_arn,
    ...
    for item in items:
        try:
            ...
            """
            Get ARN of the execution just invoked on the sync1_caller_state_machine.

...
#
# Run with:
# PYTHONPATH=.. python3 step_by_step_sync2.py
#
"""
...
"fire and forget" manner so the parent execution of the sync2_caller_state_machine
...
Note that using the resource startExecution.sync:2 causes the Output of the
nested state machine to be returned to the calling Task as JSON. If String
Output is required use startExecution.sync.
...
caller_ASL = """{
    ...
    "States": {
        "StepFunctionLauncher": {
            "Type": "Task",
            "Resource": "arn:aws:states:local::states:startExecution.sync:2",
            "Parameters": {  
                "Input.$": "$",
...
if __name__ == '__main__':
    # Initialise logger
    logger = init_logging(log_name='step_by_step_sync2')
    ...
    create_tracer("step_by_step_sync2", {"implementation": "Jaeger"})
    ...
    sync2_caller_state_machine_arn = "arn:aws:states:local:0123456789:stateMachine:sync2_caller_state_machine"
    ...
    def create_state_machines():
        ...
        try:
            ...
            response = sfn.create_state_machine(
                name="sync2_caller_state_machine", definition=caller_ASL,
            ...
            response = sfn.update_state_machine(
                stateMachineArn=sync2_caller_state_machine_arn,
    ...
    for item in items:
        try:
            ...
            """
            Get ARN of the execution just invoked on the sync2_caller_state_machine.
